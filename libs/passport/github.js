...
module.exports = new GitHubStrategy({
  ...
  function (accessToken, refreshToken, profile, done) {
    ...
    // Store id
    // FIXME
    Oauth.insert({githubUser: profile.username, profile: profile, token: bcrypt.hashSync(accessToken, 10)}, function (err) {
      if (err) {
        throw new Error(err);
      }
    });
    Oauth.findOne({githubUser: profile.username}, function (err, user) {
      var profilen = {_id: profile.id, username: profile.username};
